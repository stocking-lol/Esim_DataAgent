...
| 精通一门编程语言                                | ✓   | Python（FastAPI/Vanna 二次开发/自研 Agent 底座/327 项 pytest 工程实践）                                 |
...
| 测试与变更管理（CI/静态分析） | ✓  | **327 项 pytest**（75 个攻防用例）、评估报告、GitHub 版本管理（语义化 tag v0.1-v0.8.0） |
...
- **多租户安全隔离**：fail-closed 四层 SQL 安全网关 + RLS 行级租户隔离 + 角色级列脱敏，75 个注入攻防用例 + 327 项 pytest 全链路守护；修复注释拆分绕过真实漏洞（DRO/\*\*/P == DROP）；
...
> 我的评估是三路对照实验，且用**多轮重复评估**消除 LLM 随机性：`compare_eval3.py --trials 3` 同一评估集跑 3 轮，输出指标均值±标准差。比如自研 Agent 与 Vanna 的 EA 都是 100%，如果只看单轮可能被质疑是抽样误差，多轮取均值后波动区间给出置信边界；对差异不够显著的地方（如自愈率 16.7±2.9%（各轮 15-20% 区间）），我如实标注样本量限制，不夸大。Vanna 2.0 的 `core/evaluation` 里 `compare_agents` 做的正是这件事，我从工程上复现了它。
