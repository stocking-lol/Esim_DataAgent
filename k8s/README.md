...
- **多副本 + 就绪探针**：replicas=2，readinessProbe 保证流量只打到可服务副本；
  startupProbe 为 ChromaDB 训练数据加载留启动时间
...
- ChromaDB：清单中未包含向量库部署（生产可选用托管向量服务或独立 StatefulSet），
  本地演示时 ChromaDB 仍走 Docker Compose 提供，或在 ConfigMap 中配置独立地址。
