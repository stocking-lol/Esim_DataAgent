...
if _FRONTEND_DIR.is_dir():
    ...
    # 坑⑳：开发期前端改动必须立即可见——禁止浏览器（尤其 Edge）启发式缓存旧版
    # app.js。曾因无 Cache-Control，Chrome 新开页面拿到新版而 Edge 命中旧缓存，
    # 表现为"同一次修复一个浏览器正常一个无结果"。no-store 禁止一切缓存；
    # index.html 侧另给资源引用加 ?v= 版本号，URL 变化强制绕过残留缓存条目。
    @app.middleware("http")
